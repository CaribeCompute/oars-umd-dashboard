-- All public account roles no longer need administrator approval.
...
begin
  ...
  insert into public.profiles (
    ...
    requested_role,
    case when new.raw_user_meta_data ->> 'requested_role' in ('landowner', 'agency', 'extension_officer')
...
with activated as (
  ...
  where p.status = 'pending' and (
    ...
    (p.role in ('agency', 'extension_officer') and nullif(p.organization, '') is not null
